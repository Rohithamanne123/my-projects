  ...
  dashboardBody(
    tabItems(
      tabItem(
        ...
        fluidRow(
          column(4, selectInput("category", "Category", choices = unique([...]))),  # Dropdown for selecting expense categories.
...
server <- function(input, output, session) {
  expenses_data <- reactiveVal(data.frame(Category = character(), Amount = numeric(), Month = character()))  # Reactive value to store expenses data.
